...
function startEatGhost(ghost) {
	...
	if (!LOCK) {
		...
		pauseGhosts();
		PACPEOPLE_ARRAY.forEach((elem) => elem.pausePacman());
		//a.pausePacman()
...
function eatGhost(ghost) {
	...
	resumeGhosts();
	//a.resumePacman();
	PACPEOPLE_ARRAY.forEach((elem) => elem.resumePacman());
	LOCK = false;
}
	...
	if (eval('GHOST_' + ghost.toUpperCase() + '_MOVING === false')) {
		...
		if (canMoveGhost(ghost)) {
			...
			if (eval('GHOST_' + ghost.toUpperCase() + '_BODY_STATE === 3') && eval('GHOST_' + ghost.toUpperCase() + '_STATE != -1')) {
				if (!PACPEOPLE_ARRAY[0].PACMAN_MOVING) {
					PACPEOPLE_ARRAY[0].testGhostPacman(ghost);
				}
...
function changeDirection(ghost) {
	...
	if (state === 0 || state === 1) {
		if (ghostX != 276 && ghostY != 258) {
			var pacmanX = PACPEOPLE_ARRAY[0].PACMAN_POSITION_X;
			var pacmanY = PACPEOPLE_ARRAY[0].PACMAN_POSITION_Y;
			var axe = oneAxe();
			if (ghost === "blinky") {
